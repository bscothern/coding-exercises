...
extension API {
    ...
    // The reason the live implimentation is not here is that it can theoretically have other dependencies that might be expensive to build do its job.
    // By having it seperated out everything else can always build fast when doing normal development and only when you need the real value do you get the cost of building it all.
